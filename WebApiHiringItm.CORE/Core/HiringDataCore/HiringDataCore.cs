    ...
    public class HiringDataCore : IHiringDataCore
    {
        ...
        public async Task<IGenericResponse<HiringDataDto>> GetByIdHinringData(string contractorId, string contractId)
        {
            ...
            var hiringData = await hiringResult.Select(hd => new HiringDataDto
            {
                ...
                NombreRubro = hd.Contract.DetailContract.OrderByDescending(o => o.Consecutive).Select(s => s.RubroNavigation.Rubro).FirstOrDefault(),
                FuenteRubro = hd.Contract.DetailContract.OrderByDescending(o => o.Consecutive).Select(s => s.FuenteRubro).FirstOrDefault(),
